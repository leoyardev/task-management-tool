...
class TestCreateTask:
    def test_saves_and_returns_task(
        self, service, task_repo, project_repo, make_task, now
    ):
    ...
    def test_raises_not_found_if_project_missing(
        self, service, task_repo, project_repo, now
    ):
...
class TestCompleteTask:
    ...
    def test_notifies_task_completed_event(
        self, service, task_repo, notification, make_task
    ):
        task = make_task()
        task_repo.find_by_id.return_value = task
        task_repo.save.side_effect = lambda t: t
...
class TestLinkToProject:
    def test_links_task_to_project(
        self, service, task_repo, project_repo, make_task, make_project
    ):
        task = make_task()
        project = make_project()
    ...
    def test_raises_not_found_if_project_missing(
        self, service, task_repo, project_repo, make_task
    ):
        task = make_task()
        task_repo.find_by_id.return_value = task
        project_repo.find_by_id.return_value = None
