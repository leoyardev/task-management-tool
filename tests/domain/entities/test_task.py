...
class TestTaskCreate:
    ...
    def test_creates_with_project_linked(self, make_linked_task, project_id, project_deadline):
        task = make_linked_task(project_id=project_id, project_deadline=project_deadline)
...
class TestDeadlineConstraint:
    ...
    def test_deadline_within_project_deadline_is_valid(self, make_linked_task, project_deadline):
        task = make_linked_task(deadline=project_deadline - timedelta(days=1))
        assert task.deadline < project_deadline
 
    def test_deadline_equal_to_project_deadline_is_valid(self, make_linked_task, project_deadline):
        task = make_linked_task(deadline=project_deadline)
        assert task.deadline == project_deadline
 
    def test_deadline_exceeding_project_deadline_raises(self, make_linked_task, project_deadline):
...
class TestReopen:
    ...
    def test_event_carries_project_id_when_linked(self, make_linked_task, project_id, project_deadline):
        task = make_linked_task(project_id=project_id, project_deadline=project_deadline)
        task.mark_complete()
        task.pull_events()
        task.reopen()
...
class TestUpdate:
    ...
    def test_raises_if_deadline_exceeds_project_deadline(self, make_linked_task, project_deadline, now):
        task = make_linked_task(deadline=now + timedelta(days=5))
        with pytest.raises(DeadlineViolationError):
            task.update(deadline=project_deadline + timedelta(days=1))
 
    def test_deadline_rolled_back_after_violation(self, make_linked_task, project_deadline, now):
...
class TestLinkToProject:
    ...
    def test_raises_if_task_deadline_exceeds_project(self, make_task, project_id, project_deadline):
        task = make_task(deadline=project_deadline + timedelta(days=1))
        with pytest.raises(DeadlineViolationError):
            task.link_to_project(project_id, project_deadline)
 
    def test_valid_deadline_link_succeeds(self, make_task, project_id, project_deadline):
        task = make_task(deadline=project_deadline - timedelta(days=1))
        task.link_to_project(project_id, project_deadline)
        assert task.project_id == project_id
...
class TestUnlinkFromProject:
    ...
    def test_task_can_have_any_deadline_after_unlink(self, make_linked_task, project_deadline, now):
        task = make_linked_task(deadline=project_deadline)
        task.unlink_from_project()
        far_future = now + timedelta(days=9999)
